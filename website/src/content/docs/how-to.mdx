...
It writes references, never values, and leaves existing keys untouched. See [`init secenv`](/reference/#init-secenv) for the full key map.
...
See the dedicated runbook: [Rotate a Compromised GPG Key](/runbooks/rotate-compromised-key/). It covers the full revoke → share → rotate-at-source → verify sequence and the append-only caveats.

For a planned departure rather than a key compromise, see [Offboard a Departing Team Member](/runbooks/team-member-offboarding/) instead.
