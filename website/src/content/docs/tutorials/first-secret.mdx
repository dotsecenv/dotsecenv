...
<Aside type="tip">
New to dotsecenv? Start with the [Getting Started](/getting-started/) guide for the fastest path. This tutorial covers advanced topics like namespaces and `.secenv` file syntax.
</Aside>
...
5. **Create a `.secenv` file**
   ...
   The syntax is:
   - `VAR={dotsecenv}`: load the secret with the same name as the variable
   - `VAR={dotsecenv/}`: same as above (the trailing slash is ignored)
   - `VAR={dotsecenv/SECRET_NAME}`: load a specific secret into the variable
   - `VAR={dotsecenv/namespace::SECRET_NAME}`: load a namespaced secret

   <Aside type="tip">
   To build this file from the secrets already in your vault, run `dotsecenv init secenv` (interactive) or `dotsecenv init secenv --all`. See [`init secenv`](/reference/#init-secenv).
   </Aside>
