...
2. Collect GPG public keys from every team member
3. Add an identity to the vault for each team member
4. Share secrets with the team members who need them
...
The original value remains unchanged. A new entry is appended. That is the append-only design.
...
- [Security Model](/concepts/security-model/): understand the encryption model
