...
dotsecenv aligns with several NIST and IETF standards for cryptographic operations. This page documents two groups.
...
[FIPS 140-3](https://csrc.nist.gov/pubs/fips/140-3/final) defines security requirements for cryptographic modules. dotsecenv uses Go's native FIPS 140-3 cryptographic module, introduced in Go 1.24.
...
The version identifier determines which cryptographic module snapshot is embedded:
...
<Aside type="tip">
Without a version pin, `GOFIPS140=latest` would use whatever module ships with your Go version, potentially code that has not yet been validated. For regulated environments, always specify the version explicitly.
</Aside>

### Build It Yourself

dotsecenv is open source, and we encourage security-conscious organizations to fork and build the project on infrastructure under their direct control, whether that is air-gapped systems, internal CI/CD pipelines, or hardened build servers.
...
dotsecenv selects the appropriate hash strength based on key parameters:
...
This matches hash strength to key strength, following standard practice.
...
Modifying any field causes signature verification to fail:
...
<Aside type="caution">
For new key generation, use Ed25519 (fast, secure, small signatures) or RSA-4096 (broad compatibility). Avoid P-256 ECDSA for new deployments. P-256 is approved, but Ed25519 has better security properties.
</Aside>
...
GCM mode provides both confidentiality and integrity in a single operation. The 128-bit authentication tag detects tampering, modern CPUs accelerate AES via AES-NI, and GCM allows parallel encryption and decryption. It is also FIPS-approved (NIST SP 800-38D).
