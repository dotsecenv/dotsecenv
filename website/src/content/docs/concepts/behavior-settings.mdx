...
dotsecenv provides granular behavior settings that control how the tool handles various edge cases and error conditions.
...
<Aside type="note">
  Admins can pin any of these settings system-wide via [security policies](/concepts/security-policies/) at `/etc/dotsecenv/policy.d/` (v0.6.0+). When a user value disagrees with policy, the policy value wins and dotsecenv prints a warning to stderr.
</Aside>
...
The following behaviors have sensible defaults:
...
The GPG program path is configured explicitly:
...
<Aside type="note">
  `gpg.program` can also be pinned by [security policies](/concepts/security-policies/) (v0.6.0+) to force a specific GPG binary across a fleet.
</Aside>
