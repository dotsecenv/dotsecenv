...
What this catches is **post-hoc tampering of an existing entry**: edits or deletions are detected because the recomputed hash and signature no longer match. What it does *not* catch is a holder of a signing key appending a freshly signed entry with any `added_at` they choose — `added_at` is self-reported. See [Audit Trail → What append-only can and cannot prove](/concepts/audit-trail/#what-append-only-can-and-cannot-prove) and use git history for the strongest "when did this land in the shared repo" signal.
...
For a focused operational reference (health checks, pinentry compatibility, cache warming, troubleshooting), see [GPG Agent](/guides/gpg-agent/). The block below is the security-rationale view of the same configuration.
...
dotsecenv trusts the signer's asserted `added_at`. The signature commits the signer to that timestamp (non-repudiation of the assertion), but does not prove the entry was actually appended at that time. A legitimate key holder can backdate a fresh entry by signing it now with any `added_at`. The "Tampering" guarantee above protects existing entries from later modification; it does not protect the apparent chronology from a key holder forging plausible-looking history. For verifiable "when did this land in the shared vault," rely on the git commit timestamp — see [Audit Trail](/concepts/audit-trail/).
...
| Tampering with existing entries | Yes | Hash + signature verification         |
| Backdated insertion by key holder | No | `added_at` is self-reported; use `git log` |
