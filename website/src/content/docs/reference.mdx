...
<Aside type="note" title="Available in v0.7.0+">
...
<Aside type="note" title="Available in v0.5.1+">
  Re-added in v0.5.1 after being removed during the v0.4.0 vault subcommand restructuring.
</Aside>
...
This is useful for onboarding new team members or pre-authorizing keys before
sharing secrets. If the identity already exists in a vault it is skipped.
...
<Aside type="note" title="Available in v0.4.3+">
  List mode (calling `secret get` with no SECRET argument) was added in v0.4.3. The `--json` flag was added in v0.4.8.
</Aside>
...
Returns an array, newest-first, with `available_to` and `signed_by` per version for auditing access control across history. See [Audit Trail](/concepts/audit-trail/) for usage patterns.
...
<Aside type="note" title="Available in v0.4.0+">
  Introduced in v0.4.0 (replaces the earlier `vault list` and `vault identity list` commands).
</Aside>
...
`available_to` reflects the current authorization snapshot (the most-recent value's recipients). It is omitted for deleted secrets and secrets with no values. See [Audit Trail](/concepts/audit-trail/) for query patterns.
...
<Aside type="note" title="Available in v0.4.0+">
  Introduced in v0.4.0. The explicit `--fix` flag was added in v0.4.8 (before then, fixes were only offered through interactive prompts).
</Aside>
...
Drop superseded secret values to shrink a vault.
...
v0.4 changed the hash computation format to improve security. Secrets created with v0.3.x will fail validation with "hash mismatch" errors.

**To fix existing vaults:**

1. Each user must re-sign their own secret values (one user cannot edit another's values):

   ```bash
   # For each secret you created
   dotsecenv secret get SECRET_KEY | dotsecenv secret store SECRET_KEY
   ```

2. After re-signing, edit the vault file manually to remove old values that no longer validate. Look for entries where the `hash` field fails verification and delete those value blocks.

3. Commit the updated vault file.
...
Inspect and validate the system-wide policy directory at `/etc/dotsecenv/policy.d/`. See [Security Policies](/concepts/security-policies/) for the full design.
...
- [Security Policies](/concepts/security-policies/): admin-pinned organization-wide rules
