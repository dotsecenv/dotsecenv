...
- The Homebrew cask now installs the zsh/bash/fish shell plugins to `$(brew --prefix)/share/dotsecenv/plugin/`, matching the Linux packages

### Bug Fixes

- `secret get` now tells apart a secret that does not exist ("not found in any vault") from one that exists but your identity cannot read ("access denied", with the share command to request access)
- The shell plugin prints a fetch error once per secret; other keys mapped to the same failed secret get a one-line notice instead of a repeated error

### Other

- Homebrew install docs now include the `brew trust dotsecenv/tap` step that Homebrew 6.0 requires for third-party taps
- The Homebrew tap's README is now maintained in the monorepo and synced to the tap on release
