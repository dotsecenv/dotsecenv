...
<Tabs>
  <TabItem label="macOS (Homebrew)">
    ...
    Homebrew 6.0 requires trusting third-party taps; see the [installation guide](/tutorials/installation/) for details.
  </TabItem>
...
The end-to-end flow: initialize dotsecenv, store a secret, define a `.secenv` file with a `{dotsecenv}` placeholder, and watch the shell plugin load the decrypted value into your environment as you `cd` in.

<AsciinemaPlayer src="/demo.cast" idleTimeLimit={1.5} />
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="Install dotsecenv in CI/CD pipelines with the official GitHub Action, including signature and provenance verification."
...
dotsecenv uses hybrid encryption. Secrets are encrypted with AES-256-GCM (fast, symmetric), and the encryption key is wrapped with your GPG key (asymmetric). That means:
...
- The vault is append-only. Every operation is cryptographically signed, which gives you a built-in audit trail.
- The vault file is encrypted JSONL, so it's safe to commit to any git remote.
