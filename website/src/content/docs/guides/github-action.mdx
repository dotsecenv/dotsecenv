...
3. **Configure initialization (optional)**
   ...
   <Aside type="tip">
     Set `init-config` to an empty string `''` to run `dotsecenv init config` with defaults, or provide specific flags like `--login FINGERPRINT` or `--gpg-program /usr/bin/gpg`.
   </Aside>
...
- Release tags such as `v0.1.0` or `v1.2.3`
- Branch refs such as `main` or `develop`
- Commit SHAs such as `abc1234def5678...`
- `latest` (the default), which resolves to the most recent [release](https://github.com/dotsecenv/dotsecenv/releases)
...
<Tabs>
  ...
  <TabItem label="With Login">
    Initialize with authenticated login:

    ```yaml
    - uses: dotsecenv/dotsecenv@v0
      with:
        init-config: '--login <FINGERPRINT>'
    ```
  </TabItem>

  <TabItem label="Custom GPG">
    By default, `gpg.program` is set to `PATH`, which resolves the gpg binary from the runner's `PATH` at runtime. To pin a specific binary instead:
...
<Aside type="note" title="Key scope">
  The CI key you import can be repo-scoped (a repository secret named
  `GPG_PRIVATE_KEY`, one key per repo) or org-wide (an organization secret
  named `ORG_GPG_PRIVATE_KEY`, one key shared across repos). See
  [Key Scope](/concepts/key-scope/) for the trade-offs.
</Aside>
...
Windows runners are not supported, and support is not planned.
