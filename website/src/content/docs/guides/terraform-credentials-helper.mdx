...
<Aside type="note" title="Available in v0.4.8+">
  The `terraform-credentials-dotsecenv` wrapper script ships in v0.4.8.
</Aside>

Use dotsecenv as a [credentials helper](https://developer.hashicorp.com/terraform/internals/credentials-helpers) for Terraform and OpenTofu. Registry and backend tokens (for example, Terraform Cloud) are encrypted at rest in your vault, with no plaintext `credentials` blocks in your CLI config.

<Aside type="note">
  Credentials helpers handle **registry and backend authentication only** (for
  example, Terraform Cloud or private module registries). They are not used for
...
<Aside type="note">
  Terraform does **not** search your `PATH` for credentials helpers; it only
  looks in plugin directories. The `-plugin-dir` argument to `terraform init`
  is also ignored for credentials helpers.
</Aside>
...
| GPG passphrase prompt during `terraform init`               | GPG agent not caching            | Set `default-cache-ttl 3600` and `max-cache-ttl 14400` in `~/.gnupg/gpg-agent.conf`, then `gpgconf --reload gpg-agent`. Full walkthrough: [GPG Agent guide](/guides/gpg-agent/) |
