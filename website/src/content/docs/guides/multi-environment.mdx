...
2. Fans out a matrix job over `dev` / `staging` / `production`. Each matrix entry imports a different repo-secret-stored private key (`GPG_PRIVATE_KEY_DEV` and friends) and points at the matching vault file.
