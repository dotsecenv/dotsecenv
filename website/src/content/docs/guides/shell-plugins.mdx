...
The dotsecenv shell plugins automatically load secrets from `.secenv` files when you `cd` into a directory. When you leave, they unload automatically, keeping your environment clean.

## How It Works

1. On `cd` into a directory, the plugin checks for a `.secenv` file.
2. It runs a security check: the file must be owned by you (or root) and not world-writable.
3. You're prompted to allow loading (your choice can be remembered).
4. Plain variables are loaded first, then secrets are fetched from your vault.
5. Secrets are tree-scoped: they remain loaded in subdirectories and only unload when you leave the tree entirely.
...
<Aside type="note" title="Available in v0.5.1+">
  `dse reload` predates v0.5.1, but the "clear secret stack and re-fetch" semantics described below were introduced in v0.5.1. Older versions only re-ran the loader without clearing previously-loaded secrets.
</Aside>
...
<Aside type="note" title="Available in v0.5.0+">
...
Instead of writing these lines by hand, `dotsecenv init secenv` builds them from your vaults. In a terminal it opens a picker (one tab per vault, `space` to select); with `--all` it adds every reference at once. It writes references only, never values, and skips keys already in the file. See [`init secenv`](/reference/#init-secenv) in the CLI reference.

```bash
cd ~/my-project
dotsecenv init secenv          # pick interactively
dotsecenv init secenv --all    # add everything from your vaults
```
