...
- **Agent plugin versions:** before tagging, run `scripts/set-plugin-version.sh X.Y.Z`, `scripts/validate-agent-plugins.sh X.Y.Z`, and `git diff --check`, then commit the synchronized `.claude-plugin/plugin.json` and `.codex-plugin/plugin.json` versions. The marketplace catalog version is independent.
