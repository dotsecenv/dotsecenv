...
func (c *CLI) ensureIdentityInVault(fingerprint string, index int) *Error {
	...
	if c.Strict {
		return NewError(fmt.Sprintf("identity %s not found in vault %d and will not be added in strict mode; run 'dotsecenv vault identity add %s' first", fingerprint, index+1, fingerprint), ExitAccessDenied)
	}
