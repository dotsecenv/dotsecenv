...
| `.agents/skills/`   | Maintainer-only skills, NOT shipped to plugin installers (`changelog/`, `cli-reference-drift/`). |
| `.claude/skills`    | A symlink to `.agents/skills`, so Claude Code loads the same files Codex reads. |
...
- **`e2e-action-post-release.yml`** — POST-release smoke. Reusable workflow
...
- **`lint-workflows.yml`** — runs `actionlint` on changes under
  ...
  and `e2e-action-post-release.yml`, which are otherwise tag-triggered /
